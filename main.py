...
async def websocket_endpoint_bytes(websocket: WebSocket):
    ...
    first_text = 'Hello, Welcome to Turtle AR. Let me know if you need help!'
        ...
        while True:
            ...
            try:
                ...
                if (len(response.results[0].alternatives) <= 0):
                    ...
                    rasa_response = sendtorasa(final_response)
                    #print(f"The text sent from rasa {rasa_text}")
                    # rasa_text = rasa_response['text']
                    # rasa_response_unreal = f'''{{
                    #     "action": rasa_response['action'],
                    #     "text": rasa_response['text'],
                    #     "item_id": rasa_response['item_id'],
                    #     "position": rasa_response['position']"
                    # }}'''
                    
                    # rasa_response = json.dumps(rasa_response)
                    # print(f"The action sent from rasa {rasa_action}")
                    print(f"Response from rasa {rasa_response} {type(rasa_response)}")
                    

                    # str_response = str(rasa_response)
